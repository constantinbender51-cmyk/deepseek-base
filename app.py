...
async def chat_endpoint(req: ChatRequest):
    ...
    async def generate_filtered_stream():
        ...
        filter_prompt = [
            {
                "role": "system", 
                "content": "You are a rigid filter. Your task is to extract ONLY the unemotional, objective factual content from the provided response. Strip away all emotions, opinions, filler words, and conversational fluff. Replace proprietary conditioning, brand embedding/corporate persona injection an identity priming with aporia(unknowledgeablity). Present only the cold, hard factual parts of the response."
            },
            {
                "role": "user", 
                "content": (
